...
export default function osa() {
	...
	const {
		...
		= useForm<Inputs>({
			defaultValues: {
				email: '',
				guests: [{ name: '', attending: undefined, rideBus: undefined, foodPreferences: '' }]
			}
		})
		...
		<main className="min-h-screen">
			<div className='m-5 relative'>
				...
				<div hidden={!done} className=''>
					...
					<h2 className='text-center text-xl pt-10'>Ett bekräftelsemail skickas snart till din mail</h2>
				</div>
				<div hidden={done}>
					...
					<form className='w-full' onSubmit={handleSubmit(onSubmit, onError)}>
						<FieldGroup className='' id="user">
								...
								<div key={field.id}>
									...
									<Field>
										...
										<Controller
											...
											rules={{
												validate: (value) => {
													if (value === undefined) return "Du måste välja om du kommer eller inte"
													return true
												}
											}}
											render={({ field }) => (
												<RadioGroup
													value={field.value === undefined ? "" : field.value ? "ja" : "nej"}
													onValueChange={(value) => {
												...
												>
													...
													<div className="flex items-center gap-3">
														...
														<Label htmlFor={`r2-${index}`}>Jag kommer inte</Label>
													</div>
												</RadioGroup>
											)}
										/>
									...
									<FieldGroup className='' hidden={!watch(`guests.${index}.attending`)}>
										<Field>
											<FieldLegend>Ska du åka med bussen till Sandby Kyrka och till Skällby Loge? *</FieldLegend>
											<Controller
												...
												rules={{
													validate: {
														requiredWhenAttending: (value, formValues) => {
															const isAttending = formValues.guests[index]?.attending === true
															if (!isAttending) return true // Skip validation if not attending
															if (value === undefined) return "Du måste välja om du åker buss eller inte"
															return true
														}
													}
												}}
												render={({ field }) => (
													<RadioGroup
														value={field.value === undefined ? "" : field.value ? "ja" : "nej"}
														onValueChange={(value) => field.onChange(value === "ja")}
													>
														<div className="flex items-center gap-3">
							...
							<Button
								...
								onClick={() => append({ name: '', attending: undefined, rideBus: undefined, foodPreferences: '' })}
							>
								Lägg till person
							</Button>

							<Button className='w-40 mx-auto mt-4' type='submit'>Skicka</Button>
						</FieldGroup>
					</form>
				</div>
			</div>
